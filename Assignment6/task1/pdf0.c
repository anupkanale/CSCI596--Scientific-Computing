...
float al[3];        // Simulation box lengths in 3 directions
...
----------------------------------------------------------------------*/
  ...
  int a,ih,i,j;  // iteration variables
...
/*--------------------------------------------------------------------*/
  ...
  // Note that atoms are in a pariodic box
  fp = fopen("pos.d","r");
  fscanf(fp,"%f %f %f",&(al[0]),&(al[1]),&(al[2])); // First line is box length
  fscanf(fp,"%d",&n); // number of atoms in the next line
  r = (float*)malloc(sizeof(float)*3*n); // positions of atoms in the following lines
  for (i=0; i<n; i++)
