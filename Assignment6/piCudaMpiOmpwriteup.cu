...
int main(int argc,char **argv) {
	int myid,nproc,nbin,tid,dev_used,mpid;
	...
	nbin = NBIN/(nproc*NUM_DEVICE);  // Number of bins per MPI process
	step = 1.0/(float)(nbin*nproc*NUM_DEVICE);  // Step size with redefined number of bins

	#pragma omp parallel private(mpid,offset,sumHost,tid) reduction(+:pi)
	{
